# Unichurch Campus - Component Breakdown
...
Same component structure as North and Central campuses (identical CampusDetail template). The campus hero includes a stats overlay showing establishment year, service time, address, and a directions link. This is a variant of the standard hero-banner component.

Unichurch has an additional "Next Steps" event type not seen on other campus pages. The Event Card component should handle this without any campus-specific logic since it renders from the same events collection.

## Payload Collections Used

- **campuses** (synced from Rock, id: 4) - primary data source for campus details
